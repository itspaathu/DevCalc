...
async function digest(algo: AlgorithmIdentifier, input: string): Promise<string> {
...
// MD5 implementation (pure JS, since Web Crypto does not support MD5)
export function md5(input: string): string {
  ...
  for (let chunk = 0; chunk < withOne.length; chunk += 64) {
    ...
    for (let i = 0; i < 16; i++) {
      M[i] = dv.getInt32(chunk + i * 4, true)
    }
    let A = a0
    let B = b0
    let C = c0
